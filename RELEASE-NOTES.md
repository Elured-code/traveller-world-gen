...
`StarCluster.member_stars: List[str]` — the spectral classifications of the other
`system_count − 1` systems in the cluster's centre hex, generated by new
`_roll_cluster_member(age_gyr)` inside `_roll_star_cluster_meta()`.

**WBH compliance fix:** `_roll_cluster_member()` now returns `"BD"` when the Star Type
table roll lands on the Special row — per WBH p.219, a nested Unusual result within a
cluster is treated as a brown dwarf, not re-entered into the Unusual column. Previously
it fell through to `"M"` (incorrect).

Schema v2.1.4: `member_stars` array added to `StarCluster` `$def` in
`traveller_system_schema.json`; added to `required` list. 5 new tests in
`TestMemberStars` (count, format, round-trip, determinism; format test updated to accept
`"BD"`).
