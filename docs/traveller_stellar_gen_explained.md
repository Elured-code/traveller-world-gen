...
class StarSystem:
    ...
    star_cluster: Optional[StarCluster]  # None unless primary is Star Cluster type

@dataclass
class StarCluster:
    age_gyr: float         # Cluster age in Gyr (same formula as Nebula; max 1.80)
    single_hex: bool       # True when cluster fits in one hex (1D < 4)
    hex_diameter: int      # 1 if single_hex; else 1D+1 (range 2–7)
...
When the primary is a **Star Cluster** environment type (WBH p.219, issue #182),
`StarSystem.star_cluster` holds the cluster metadata generated by
`_roll_star_cluster_meta()`. For all other primary types, `star_cluster` is `None`.
...
generate_stellar_data()   →  StarSystem
        │                     └─ .star_cluster: Optional[StarCluster]
        ▼
generate_orbits(star_system, cluster=star_cluster)  →  SystemOrbits
```
