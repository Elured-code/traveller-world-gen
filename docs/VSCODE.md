...
before configuring VS Code. The project venv is named `.venv` (not `.venv`).
...
| `python.analysis.venv` | Names the specific venv to use (`.venv`) |
...
remains `.venv/bin/pylint <file>` in the terminal.
...
PySide6 must be installed in `.venv`:

```bash
.venv/bin/pip install -r gen-ui/requirements.txt
```
