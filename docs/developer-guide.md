...
class World:
    ...
    atmosphere_detail: Optional[AtmosphereDetail] = None  # WBH pp. 78-82, Session 31
    temperature: str
...
**`AtmosphereDetail` (WBH pp. 78–82, Session 31):** quantitative atmosphere
...
Generated by `generate_atmosphere_detail(code, size, system_age_gyr=None)`
and attached to `World.atmosphere_detail` by the orchestrator. JSON output
nests non-None fields under `"atmosphere"."detail"` and adds a WBH p.82
profile string under `"atmosphere"."profile"` (e.g. `"6-1.013-0.212"`).
The profile is derived from the dataclass on demand via
`format_atmosphere_profile(code, detail)` — kept as a free function so
later WBH phases (taints, exotic-atmosphere subtypes, gas composition,
altitude limits) can extend the format without further dataclass churn.
