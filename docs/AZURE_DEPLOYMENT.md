...
|-----------|-----------|-------|------|---------|-------------|
| `detail` | `/system`, `/system/{name}`, `/system/{name}/card`, `/map/system`, `/map/system/{name}` | query / body | boolean | `false` | When `true`, generate secondary world SAH/social profiles and satellite data for every orbit slot and moon. Adds ~10–50 ms depending on system size. |
| `format` | `/system/full`, `/map/system`, `/map/system/{name}` | query / body | string | `json` | Output format: `json`, `html`, or `text`. |
...
| `hex` | `/map/system` | query / body | string | 4-digit hex position, e.g. `1910`. Use with `sector` to locate a world precisely without searching by name. |
...
| `canonical_profile` | string | Present on TravellerMap systems only; the canonical UWP of the mainworld placed in this orbit, e.g. `"A788899-C"`. Takes display precedence over `detail.profile` for this slot. |
