...
from traveller_gen.traveller_belt_physical import (
    BeltPhysical,
    _roll_belt_span,
    _roll_bulk,
    _roll_composition,
...
class TestRollComposition:
    def test_inner_hz_dm_minus4(self):
        ...
        with patch("traveller_gen.traveller_belt_physical.random.randint", return_value=1):
    ...
    def test_normalisation_trims_m_first(self):
        ...
        with patch("traveller_gen.traveller_belt_physical.random.randint", return_value=6):
            m, s, c, other = _roll_composition(hz_deviation=-1.0)
        assert m + s + c <= 100
        assert other == 0
    ...
    def test_row_11_m_uses_d3(self):
        ...
        dice = iter([4, 3, 6, 1, 1])
        with patch("traveller_gen.traveller_belt_physical.random.randint", side_effect=dice):
            m, s, c, other = _roll_composition(hz_deviation=3.0)
        assert c == 65
...
class TestRollBeltSpan:
    def test_no_dms(self):
        ...
        with patch("traveller_gen.traveller_belt_physical.random.randint", return_value=3):
...
class TestRollBulk:
    ...
    def test_age_dm_rounds_down(self):
        ...
        with patch("traveller_gen.traveller_belt_physical.random.randint", return_value=2):
...
class TestRollResourceRating:
    ...
    def test_c_pct_dm_negative_rounding(self):
        ...
        with patch("traveller_gen.traveller_belt_physical.random.randint", return_value=4):
