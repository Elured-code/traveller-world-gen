...
class Star:
    ...
    name: str = ""                    # set by attach_body_names(); e.g. "Homeworld-Primary"
...
class OrbitSlot:
    ...
    notes: str
    name: str = field(default="", init=False)  # set by attach_body_names(); e.g. "Homeworld-A"
...
class World:
    ...
    population_detail: Optional["PopulationDetail"] = field(default=None, init=False)
                                    # WBH Social Characteristics population profile.
                                    # Set by attach_population_detail() (Session 95, issue #95).
                                    # None by default; only set when "Population detail" option enabled.
                                    # Emitted in to_dict() only when not None.
    government_detail: Optional["GovernmentDetail"] = field(default=None, init=False)
                                    # WBH Social Characteristics government profile.
                                    # Set by attach_government_detail() (Session 99, issue #96).
                                    # None by default; only set when "Government detail" option enabled.
                                    # Emitted in to_dict() only when not None.
                                    # None for gov code 0 or 7 (Balkanised — issue #130).
    culture_detail: Optional["CultureDetail"] = field(default=None, init=False)
                                    # WBH Social Characteristics cultural profile (Diversity, Xenophilia,
                                    # Uniqueness, Symbology, Cohesion, Progressiveness, Expansionism,
                                    # Militancy + 4-char DXUS-CPEM cultural profile string).
                                    # Set by attach_culture_detail() (Session 127, issue #99).
                                    # Only set when `want_social_detail` flag is True.
    importance_detail: Optional["WorldImportance"] = field(default=None, init=False)
                                    # CRB Social Characteristics world importance score (signed integer
                                    # + 8 component DMs: starport, population, tech level, Ag/In/Ri trade
                                    # codes, bases, waystation).
                                    # Set by attach_importance_detail() (Session 132, issue #155).
                                    # Deterministic; always computed when social detail is enabled.
                                    # Only for mainworlds (population > 0).
...
### `WorldImportance` (`traveller_world_importance.py`)

```python
@dataclass
class WorldImportance:  # pylint: disable=too-many-instance-attributes
    # --- Importance score (Session 132) ---
    importance:            int            # signed total score (range −3 to +6)
    starport_dm:           int            # −1, 0, or +1 (A/B=+1, C=0, D/E/X=−1)
    population_dm:         int            # −1, 0, or +1 (≤6=−1, 7–8=0, ≥9=+1)
    tech_dm:               int            # −1, 0, +1, or +2 (≤8=−1, 9=0, 10–15=+1, ≥16=+2)
    agricultural_dm:       int            # 0 or +1 (Ag trade code)
    industrial_dm:         int            # 0 or +1 (In trade code)
    rich_dm:               int            # 0 or +1 (Ri trade code)
    base_dm:               int            # 0 or +1 (two or more non-Corsair bases)
    waystation_dm:         int            # 0 or +1 (X-Boat waystation "W" base code)
    # --- Economic characteristics (Session 133, issue #100) ---
    ...
    efficiency_factor:     Optional[int]  # −5 to +5 (0 internally treated as +1); None before attach
    resource_units:        Optional[int]  # RF × LF × IF × EF (zeros treated as 1); None before attach
    gwp_base:              Optional[int]  # IF_adj + min(RF_adj, IF_adj); None before attach
    gwp_per_capita:        Optional[int]  # GWP per capita in Credits; None before attach
...
Generated by `generate_importance_detail(starport, population, tech_level,
trade_codes, bases)`. Fully deterministic (no dice). Mainworld only; applied
by `attach_importance_detail()` which skips if mainworld is None or
population=0. The importance score is the sum of all eight component DMs.

**Economic characteristics** (Session 133, issue #100) are computed by
`attach_importance_detail()` using the economic factor functions:

| Function | Formula |
|---|---|
| `compute_efficiency_factor()` | pop 0→−5; pop 1–6→2D6−7+DMs; pop 7+→2D3−4+DMs; clamped −5/+5; 0→+1 |
| `compute_resource_units()` | RF×LF×IF×EF; zero factors treated as 1; only EF can be negative |
| `compute_gwp_base()` | IF_adj + min(RF_adj, IF_adj); both adjusted to ≥1 |
| `compute_gwp()` | base×TL×port×gov×TC mods; EF>0 multiplies; EF<0 divides by (1−EF) |
| `compute_development_score()` | (GWP_pc/1000) × (1 − IR/100); IR defaults 0 until implemented |
| `compute_economics_profile()` | `{RF_eHex}{LF_eHex}{IF_eHex}{EF_signed}` e.g. `"765+2"` |

EF government DMs: −1 for gov {0,3,6,9,B,C,F}; +1 for gov {1,2,4,5,8}. Also
law level −1 per LL > 5, PCR +PCR, progressiveness DM, expansionism DM.

Displayed in `world_card.html` culture detail section: importance row (bold
when > +3), then labour/infrastructure/resource factor rows, then EF, RU, GWP
base, GWP per capita, total GWP, development score, economics profile (highlighted).
...
class WorldPhysical:    # pylint: disable=too-many-instance-attributes
    ...
    # Adjusted resource factor for economics (WBH p.131, Session 133). Set by
    # attach_resource_factor() in system_pipeline.py after apply_mainworld_social()
    # (requires tech_level and trade_codes). Formula: resource_rating + TL_DM + trade_DMs.
    # TL_DM: TL 9-12 → +1, TL 13-15 → +2, TL 16+ → +3.
    # Trade DMs: Ag −1, In +2, As −1, Ni −1, Ri +1.
    # Clamped to [0, 12]. Emitted in to_dict() only when not None.
    # Also exposed on World.to_dict() as top-level "resource_factor" key.
    ...
    #       [, stellar_day_hours][, resource_rating][, resource_factor]  ← all only when not None
```
...
class BeltPhysical:    # pylint: disable=too-many-instance-attributes
    ...
    # computed property: .profile_str -> str
    # Returns WBH Class III shorthand: "S-CC.CC.CC.CC-B-R-#-s"
    # S = round(outer_au - inner_au, 3) (span in AU)
    # CC.CC.CC.CC = m_type_pct.s_type_pct.c_type_pct.other_pct (four components dot-separated)
    # B = bulk, R = resource_rating, # = size_1_bodies, s = size_s_bodies
    # Displayed in Notes column of system detail (to_html / system_body_table) (Session 123)
...
class WorldDetail:
    ...
    is_independent_government: bool  # True when generated with Case 2 (Session 89, issue #17)
                                     # emitted in to_dict() only when True; defaults False
    native_sophont: bool             # True when native sophont confirmed (biocomplexity≥8)
                                     # set by _set_biocomplexity() in _apply_biomass()
                                     # (Session 92, issue #125); emitted in to_dict() only when True
    classification: Optional[str]    # Two-letter WBH p.163 code: "Cy", "Fa", "Fp", "Mb",
                                     # "Mi", "Pe", "Rb"; None when no classification qualifies.
                                     # (Session 94, issue #18); emitted in to_dict() only when not None.
                                     # Code is also appended to trade_codes.
    population_detail: Optional[object]  # traveller_world_population_detail.PopulationDetail
                                     # Set by attach_population_detail() (Session 95, issue #95).
                                     # None by default; emitted in to_dict() only when not None.
    government_detail: Optional[object]  # traveller_world_government_detail.GovernmentDetail
                                     # Set by attach_government_detail() (Session 99, issue #96).
                                     # None by default; emitted in to_dict() only when not None.
    name: str                        # set by attach_body_names() (Session 102, issue #131).
                                     # Mirrors the parent OrbitSlot.name or Moon.name.
                                     # "" by default; emitted in to_dict() only when non-empty.
    ...
    #                        includes "is_independent_government": true when True
    #                        includes "classification": str when not None
    #                        includes "population_detail": dict when not None
```
...
class Moon:  # pylint: disable=too-many-instance-attributes
    ...
    name: str = field(default="", init=False)  # set by attach_body_names(); e.g. "Homeworld-A-alpha"
                                              # rings are skipped; name stays "" for ring moons
