...
| World physical detail beyond basic | pp. 78–130 | Partially done. Diameter, density, composition, mass, gravity, escape velocity, axial tilt, day length (`traveller_world_physical.py`). Atmosphere detail Phases 1–5 (`traveller_world_gen.py`). Hydrographic detail Phase 1 — surface liquid % (`traveller_hydro_detail.py`, Session 37). Seismic stress (RSS, THF, TSS, seismic temperature, Session 56). Surface tidal amplitude in metres (Session 58, issue #68). Tidal Stress Factor (WBH p.126) deferred as issue #67. Remaining: hydrographic composition beyond surface liquid % (ocean type, ice caps, depth), native life ratings. |
