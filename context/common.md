...
All generation modules live in the `src/traveller_gen/` Python package (Session 134).
Install with `pip install -e .` for development; `pip install --target azure-api/ --no-deps .` for deployment.
...
│   ├── function_app.py          # Azure Functions v2 — all 11 HTTP endpoints
│   ├── host.json                # Azure Functions host configuration (rate limiting)
│   ├── local.settings.json.example
│   ├── requirements.txt         # azure-functions, jsonschema, Jinja2 (traveller-gen bundled separately)
│   └── shared/
│       └── helpers.py           # Request parsing, response builders, error codes
│
├── fastapi/                     # FastAPI + uvicorn server (same 11 endpoints, no auth)
│   ├── app.py                   # FastAPI app — all 11 HTTP endpoints + SlowAPI rate limit
│   ├── helpers.py               # FastAPI-native parsing, response builders, error codes
│   ├── requirements.txt         # fastapi, uvicorn, slowapi, jsonschema, Jinja2
│   └── local.settings.json.example
│
├── gen-ui/
...
│   ├── test_function_app.py         # Azure Functions API endpoints
│   └── test_fastapi_app.py          # FastAPI server endpoints (TestClient, no stubs needed)
├── conftest.py                  # pytest configuration + azure.functions stub
│
...
2613 tests total; all must pass before committing.
