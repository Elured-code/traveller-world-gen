...
class MapWorldData:
    ...
    worlds: int = 0     # total world count from TravellerMap
    cx: str = ""        # T5 Cultural Extension, 4 eHex chars, e.g. "7567"
    importance: int = 0 # T5 Importance Extension integer, e.g. +2 or -1
```
...
- `"Stars"` / `"Stellar"` / `"Star"`
- `"PBG"` / `"Pbg"`
- `"Cx"` / `"CulturalExtension"` (Session 130)
- `"Ix"` / `"ImportanceExtension"` / `"Importance"` (Session 130)
...
5b. Stamp T5 Cultural Extension on mainworld (Session 130) — if `map_data.cx`
   is non-empty, set dynamic attributes `world.cx` and `world.importance`.
   These are used later by `attach_culture_detail()` to route to the Cx
   conversion path (see `social-detail.md`).
...
7. `generate_temperature_from_orbit(atmosphere, hz_deviation)` — temperature from
   orbital position (temperature is not stored in TravellerMap UWP).
   `hzco` and `orbit` parameters were removed in Session 116; call with only
   `atmosphere` and `hz_deviation`.
